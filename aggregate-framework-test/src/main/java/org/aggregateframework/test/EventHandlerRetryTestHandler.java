...
class MethodEventCallCounter {
    public AtomicInteger callCount;
    public Integer expectFailCount;
}

class EventHandlerRetryTestHandler {

    static final Logger logger = Logger.getLogger(EventHandlerRetryTestHandler.class.getSimpleName());

    private Map<String, Map<Object, MethodEventCallCounter>> tryCount = new LinkedHashMap<String, Map<Object, MethodEventCallCounter>>();
    ...
    */
    ...
    public void onAfterHandler(EventHandlerRetryConfig config, Object target, Method method, Object[] args, Exception e) throws IllegalAccessException {
        synchronized (this) {
            Object eventKey = getKeyFromEvent(config.isShowDebugMessage(), config.getEventUniqueKey(), args);
            MethodEventCallCounter counter = getCallCount(target, method, eventKey, config.failCount);
            ...
            if (config.isShowDebugMessage()) {
                ...
                try {
                    Thread.sleep(5); // 等待，防止上面打印log信息的时候被抛异常的信息打乱
    ...
    private StringBuffer buildCallMethodInfo(EventHandlerRetryConfig config, String str, Object arg, Object eventKey, MethodEventCallCounter counter, int registCalls) {
        StringBuffer sb = new StringBuffer();
        sb.append(bannerStart)
